...
const AddForm = (props) => {
            ...
            try {
                if (editId !== 0) {
                    const getCategoryData = async () => {
                        try {
                            const parentData =
                                await dashboardService.getCategoryForParentSelect(
                                    editId.id
                                );
                            setData(parentData);
                    ...
                    setFileList([
                        {
                            ...
                            name: editId.image1,
                            status: "done",
                            url: `${API_URL}uploads/${editId.image1}`,
                        },
                        {
                            uid: "-2",
                            name: editId.image2,
                            status: "done",
                            url: `${API_URL}uploads/${editId.image2}`,
                        },
                        {
                            uid: "-3",
                            name: editId.image3,
                            status: "done",
                            url: `${API_URL}uploads/${editId.image3}`,
                        },
                    ]);
                } else {
                    setData(initialData);
    ...
    const handleAdd = async (value) => {
        if (fileList.length < 3) {
            ...
            try {
                await dispatch(
                    addData("coupon_category", payload, editId !== 0)
                );
                dispatch(hideDrawer());
                setLoading(false);
            } catch {
